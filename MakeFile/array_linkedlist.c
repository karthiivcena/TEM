...
void memset(char *ptr,char c, unsigned long int size);
...
int array_list_main()
{
    ...
    for(i=9; i>=0; i--)
    {
    	free_pool_node(&rlist_head, node[i]);                     /* Calling the Free Pool Function */
    }       
    display_rpool(rlist_head);                                    /* Calling the Display Function */
    return 0;
}
...
void mem_set(char *ptr,char c, unsigned long int size)
{
