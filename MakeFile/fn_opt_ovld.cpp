...
Bytes::Bytes(unsigned int k)
{
    int index;
    this->byte = (unsigned char) k;
}

Bytes::Bytes(unsigned char ch)
{
    this->byte = ch;
}

Bytes::Bytes(char* inpStr)
{    
...
char* Bytes::trim(char *inStr, char *outStr) 
{
    int f_ind = -1;
    int l_ind = -1;
    int i;
...
/* setBit Function */
int Bytes::setBit(unsigned int pos, unsigned int value)
{   
...
char* Bytes::getBitStr(char* bitStr)
{
    int i;
    for(i = 7;i >= 0; i--)
    {
...
int Bytes::clearBit(unsigned int pos)
{
    if(pos>7)
        return STATUS_FAILURE;
    byte = (byte & (~ (0x01 << (pos))));
...
Bytes operator + (Bytes &o1, Bytes &o2)
{
